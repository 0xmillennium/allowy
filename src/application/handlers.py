...
async def handle_initialize_application(
    ...
    """Seeds IP sources from config into the database, then registers all sources with the scheduler."""
    logger.debug("Initializing application", extra={"source_count": len(command.sources)})
        ...
        if entry.source_type in fetcher.supported_source_types():
            async with uow:
                if not await uow.ip_sources.get_by_url(SourceUrl(value=entry.url)):
                    if not await uow.ip_sources.get_by_name(SourceName(value=entry.name)):
                        ...
                        logger.info("Seeded IP source", extra={"source_id": source.id.value})
                    else:
                        logger.warning("Skipping seed, name already taken", extra={"source_name": entry.name})
                else:
                    logger.info("Skipping seed, URL already exists", extra={"source_url": entry.url})
        else:
            logger.warning("Skipping unsupported source type", extra={"source_type": entry.source_type})
...
async def handle_create_ip_source(
    ...
    logger.debug("Creating IP source", extra={"source_name": command.source.name, "source_type": command.source.source_type})
    ...
    async with uow:
        if await uow.ip_sources.get_by_url(SourceUrl(command.source.url)):
            raise IpSourceAlreadyExistsException()
    ...
    logger.info("Created IP source", extra={"source_id": source.id.value, "source_name": command.source.name})
...
async def handle_update_sync_interval(
    ...
    async with uow:
        ...
        if not source:
            raise IpSourceNotFoundException()
    ...
    logger.info("Updated sync interval", extra={"source_id": command.source_id, "new_interval": command.sync_interval})
...
async def handle_update_source_name(
    ...
    logger.info("Updated source name", extra={"source_id": command.source_id, "new_name": command.name})
...
async def handle_update_source_type(
    ...
    logger.info("Updated source type", extra={"source_id": command.source_id, "new_type": command.source_type})
...
async def handle_sync_ip_source(
    ...
    logger.info("Synced IP source", extra={"source_id": command.source_id, "range_count": len(ranges)})
...
async def handle_ip_ranges_updated(
    ...
    """Aggregates IP ranges from all active sources and writes formatted output files."""
    async with uow:
        sources = await uow.ip_sources.get_all()
        active_ranges = set()
        for source in sources:
            if source.is_active:
                active_ranges.update(
                    ip_range.cidr for ip_range in source.ip_ranges
                )

    for formatter in FORMATTERS:
        output = formatter.format(list(active_ranges))
        await filer.write(output.content, output.filename)
    logger.info("Wrote output files", extra={"formatter_count": len(FORMATTERS), "active_range_count": len(active_ranges)})
