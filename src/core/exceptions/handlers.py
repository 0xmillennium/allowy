...
async def handle_request_validation_error(request, exc: RequestValidationError) -> JSONResponse:
    errors = exc.errors()
    logger.error("Request validation error", extra={"errors": errors, "method": request.method, "path": request.url.path})
    return JSONResponse(
        status_code=422,
        content={
            "code": 422,
...
async def handle_validation_error(request, exc: ValidationError) -> JSONResponse:
    errors = exc.errors()
    logger.error("Validation error", extra={"errors": errors, "method": request.method, "path": request.url.path})
    return JSONResponse(
        status_code=422,
        content={
            "code": 422,
...
async def handle_sqlalchemy_error(request, exc: SQLAlchemyError) -> JSONResponse:
    logger.error("Database error", extra={"error": str(exc), "method": request.method, "path": request.url.path})
    return JSONResponse(
        status_code=500,
        content={
            "code": 500,
...
async def handle_http_exception(request, exc: HTTPException) -> JSONResponse:
    logger.error("HTTP error", extra={"status_code": exc.status_code, "detail": exc.detail, "method": request.method, "path": request.url.path})
    return JSONResponse(
...
async def handle_app_exception(request, exc: AppException) -> JSONResponse:
    logger.error("Application error", extra={"error_type": exc.type, "error_msg": exc.msg, "method": request.method, "path": request.url.path})
    return JSONResponse(
...
async def handle_generic_error(request, exc: Exception) -> JSONResponse:
    logger.error("Unexpected error", extra={"error": str(exc), "error_type": type(exc).__name__, "method": request.method, "path": request.url.path})
    return JSONResponse(
        status_code=500,
        content={
            "code": 500,
